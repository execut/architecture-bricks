...
func (r *Repository) Update(
	...
	tag, err := tx.Exec(ctx, `
        UPDATE product
        SET name = $1, status = $2, approve_reason = $3, rejection_reason = $4, version = version + 1
        WHERE id = $5 AND version = $6
    `, product.Name(), string(product.Status()), approveReason, rejectionReason, product.ID(), product.Version())
	if err != nil {
		return err
	}
...
func (r *Repository) GetByID(ctx context.Context, id string) (*domain.Product, error) {
	...
	err := r.pool.QueryRow(ctx, `
		...
		FROM product
		WHERE id = $1
	`, id).Scan(&productID, &productName, &userID, &status, &approveReason, &rejectionReason, &version)
	if err != nil {
