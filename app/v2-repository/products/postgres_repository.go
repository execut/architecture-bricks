...
func (r *PostgresRepository) UpdateProduct(
    ...
    if product.Status == contract.ProductStatusApproved {
        _, err = tx.Exec(ctx, `
            ...
            WHERE id = $4
        `, product.Name, string(product.Status), string(product.ApproveReason), product.ID)
    } else {
        _, err = tx.Exec(ctx, `
            UPDATE product
            SET name = $1
...
func (r *PostgresRepository) GetProduct(ctx context.Context, productID string) (contract.Product, error) {
    product := contract.Product{}
    var status string
    var approveReason, rejectionReason *string
    err := r.pool.QueryRow(ctx, `
        ...
        FROM product
        WHERE id = $1
    `, productID).Scan(&product.ID, &product.Name, &product.UserID, &status, &approveReason, &rejectionReason)
    if err != nil {
...
func (r *PostgresRepository) ApproveProduct(
    ...
    tag, err := tx.Exec(ctx, `
        ...
        WHERE id = $3
    `, string(contract.ProductStatusApproved), string(contract.ProductApproveReasonModerator), productID)
    if err != nil {
        return err
    }
...
func (r *PostgresRepository) RejectProduct(
    ...
    tag, err := tx.Exec(ctx, `
        ...
        WHERE id = $3
    `, string(contract.ProductStatusRejected), string(contract.ProductRejectionReasonModerator), productID)
    if err != nil {
        return err
    }
